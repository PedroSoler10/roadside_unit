...
class ScenarioClassificator(Node):
    def __init__(self):
        ...
        self.visualization_flag = self.declare_parameter('scenario_visualization_flag', False).value

        self.scenario_publisher = self.create_publisher(String, '/scenario', 10)
        ...
        self.scenario_publisher.publish(String(data=self.scenario))
        self.detections_subscriber = self.create_subscription(Detection2DArray, detections_topic, self.detections_callback, 10)
    ...
    def detections_callback(self, data):
        ...
        if self.visualization_flag and (self.scenario != "free" or self.scenario != "unknown"):
